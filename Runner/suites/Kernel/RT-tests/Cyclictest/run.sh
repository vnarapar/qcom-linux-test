...
#
# CyclicTest wrapper for qcom-linux-testkit
# - Runs rt-tests cyclictest ITERATIONS times (JSON output)
# - Parses KPI using lib_rt.sh (no python required)
# - Emits KPI lines to result.txt and summary PASS/FAIL/SKIP to CyclicTest.res
#
# Notes:
# - Always exits 0 (LAVA-friendly). Use CyclicTest.res for gating.
...
log_info "$TESTNAME: Checking for the tools required to run cyclictest"
...
CYCLICTEST_BIN=$(rt_resolve_binary cyclictest "$BINARY" 2>/dev/null || echo "")
if [ -z "$CYCLICTEST_BIN" ] || [ ! -x "$CYCLICTEST_BIN" ]; then
  log_skip "$TESTNAME: cyclictest binary not found/executable (${CYCLICTEST_BIN:-none})"
  echo "$TESTNAME SKIP" >"$RES_FILE"
  exit 0
fi

rt_log_common_runtime_env "$TESTNAME" "$CYCLICTEST_BIN"
log_info "$TESTNAME: iterations=$ITERATIONS duration=$DURATION interval=$INTERVAL threads=$THREADS prio=$PRIO"
...
while [ "$i" -le "$ITERATIONS" ] 2>/dev/null; do
  ...
  stdoutlog="${OUT_DIR}/cyclictest_stdout_iter${i}.log"
  ...
  set -- "$@" -p "$PRIO" -i "$INTERVAL" -t "$THREADS" -D "$DURATION" --json="$jsonfile"
  ...
  if [ "$rc" -ne 0 ] 2>/dev/null; then
    log_fail "$TESTNAME: cyclictest exited rc=$rc (iter $i/$ITERATIONS)"
    overall_fail=1
  fi
  ...
  if ! rt_parse_and_append_iteration_kpi "cyclictest" "$jsonfile" "$TMP_ONE" "$ITER_KPI" "$RESULT_TXT" "$i"; then
...
rt_emit_aggregate_kpi "$TESTNAME" "cyclictest" "$ITER_KPI" "$AGG_KPI" "$RESULT_TXT" || true
rt_emit_thread_aggregate_kpi "$TESTNAME" "cyclictest" "$ITER_KPI" "$THREAD_AGG_KPI" "$RESULT_TXT" || true

if rt_kpi_file_has_fail "cyclictest" "$ITER_KPI"; then
  overall_fail=1
fi
