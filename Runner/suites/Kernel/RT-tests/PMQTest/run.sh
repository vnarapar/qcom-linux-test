...
#
# PMQTest wrapper for qcom-linux-testkit
# - Runs rt-tests pmqtest ITERATIONS times (JSON output)
# - Parses KPI using lib_rt.sh (no python required)
# - Emits KPI lines to result.txt and summary PASS/FAIL/SKIP to PMQTest.res
#
# Notes:
# - Always exits 0 (LAVA-friendly). Use PMQTest.res for gating.
...
log_info "$TESTNAME: Checking for the tools required to run pmqtest"
...
PMQ_BIN=$(rt_resolve_binary pmqtest "$BINARY" 2>/dev/null || echo "")
if [ -z "$PMQ_BIN" ] || [ ! -x "$PMQ_BIN" ]; then
  log_skip "$TESTNAME: pmqtest binary not found/executable (${PMQ_BIN:-none})"
  echo "$TESTNAME SKIP" >"$RES_FILE"
  exit 0
fi

rt_log_common_runtime_env "$TESTNAME" "$PMQ_BIN"
...
while [ "$i" -le "$ITERATIONS" ] 2>/dev/null; do
  ...
  stdoutlog="${OUT_DIR}/pmqtest_stdout_iter${i}.log"
  ...
  if [ "$rc" -ne 0 ] 2>/dev/null; then
    log_fail "$TESTNAME: pmqtest exited rc=$rc (iter $i/$ITERATIONS)"
    overall_fail=1
  fi
  ...
  if ! rt_parse_and_append_iteration_kpi "pmqtest" "$jsonfile" "$TMP_ONE" "$ITER_KPI" "$RESULT_TXT" "$i"; then
...
rt_emit_aggregate_kpi "$TESTNAME" "pmqtest" "$ITER_KPI" "$AGG_KPI" "$RESULT_TXT" || true
rt_emit_thread_aggregate_kpi "$TESTNAME" "pmqtest" "$ITER_KPI" "$THREAD_AGG_KPI" "$RESULT_TXT" || true

if rt_kpi_file_has_fail "pmqtest" "$ITER_KPI"; then
  overall_fail=1
fi
