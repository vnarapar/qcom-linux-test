...
#
# PTSEMATest wrapper for qcom-linux-testkit
# - Runs rt-tests ptsematest ITERATIONS times (JSON output)
# - Parses KPI using lib_rt.sh (no python required)
# - Emits KPI lines to result.txt and summary PASS/FAIL/SKIP to PTSEMATest.res
#
# Notes:
...
log_info "$TESTNAME: Checking for the tools required to run ptsematest"
...
PSEMA_BIN=$(rt_resolve_binary ptsematest "$BINARY" 2>/dev/null || echo "")
if [ -z "$PSEMA_BIN" ] || [ ! -x "$PSEMA_BIN" ]; then
  log_skip "$TESTNAME: ptsematest binary not found/executable (${PSEMA_BIN:-none})"
  echo "$TESTNAME SKIP" >"$RES_FILE"
  exit 0
fi

rt_log_common_runtime_env "$TESTNAME" "$PSEMA_BIN"
...
while [ "$i" -le "$ITERATIONS" ] 2>/dev/null; do
  ...
  stdoutlog="${OUT_DIR}/ptsematest_stdout_iter${i}.log"
  ...
  if [ "$rc" -ne 0 ] 2>/dev/null; then
    log_fail "$TESTNAME: ptsematest exited rc=$rc (iter $i/$ITERATIONS)"
    overall_fail=1
  fi
  ...
  if ! rt_parse_and_append_iteration_kpi "ptsematest" "$jsonfile" "$TMP_ONE" "$ITER_KPI" "$RESULT_TXT" "$i"; then
...
rt_emit_aggregate_kpi "$TESTNAME" "ptsematest" "$ITER_KPI" "$AGG_KPI" "$RESULT_TXT" || true

if rt_kpi_file_has_fail "ptsematest" "$ITER_KPI"; then
  overall_fail=1
fi
