...
if ! check_dependencies weston-scaler; then
  log_fail "Required binary weston-scaler not found in PATH."
  echo "$TESTNAME FAIL" >"$RES_FILE"
  exit 0
fi

BIN=$(command -v weston-scaler)
log_info "Using weston-scaler: $BIN"
