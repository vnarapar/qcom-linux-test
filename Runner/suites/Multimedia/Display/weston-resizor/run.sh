...
if ! check_dependencies weston-resizor; then
  log_fail "Required binary weston-resizor not found in PATH."
  echo "$TESTNAME FAIL" >"$RES_FILE"
  exit 0
fi

BIN=$(command -v weston-resizor)
log_info "Using weston-resizor: $BIN"
