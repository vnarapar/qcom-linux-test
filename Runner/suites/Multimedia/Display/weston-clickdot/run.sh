...
if ! check_dependencies weston-clickdot; then
  log_fail "Required binary weston-clickdot not found in PATH."
  echo "$TESTNAME FAIL" >"$RES_FILE"
  exit 0
fi

BIN=$(command -v weston-clickdot)
log_info "Using weston-clickdot: $BIN"
